...
func TestFindEnvFilePathAtPosition(t *testing.T) {
	tests := []struct {
		name     string
		text     string
		line     int
...
env_files:
  ...
  - .env.prod`,
		...
		{
			name: "cursor on second env file",
			text: `yapi: v1
url: https://example.com
env_files:
  - .env.local
...
url: https://example.com`,
		...
		{
			name: "cursor at start of env file path",
			text: `yapi: v1
env_files:
  - .env.test`,
			line:     2,
		...
		{
			name: "cursor at end of env file path",
			text: `yapi: v1
env_files:
  - .env.test`,
			line:     2,
		...
		{
			name: "cursor past end of env file path",
			text: `yapi: v1
env_files:
  - .env.test`,
			line:     2,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := findEnvFilePathAtPosition(tt.text, tt.line, tt.char)
			if result != tt.expected {
				t.Errorf("findEnvFilePathAtPosition() = %q, want %q", result, tt.expected)
			}
		})
	}
}
